...
export class EtaxAdapter {
  ...
  async getStatutFiscal(nif: string): Promise<StatutFiscalResult> {
    ...
    return this.cb.execute(
      this.CIRCUIT,
      async () => {
        const res = await fetch(`${this.endpoint()}/contribuables/${nif}/statut`, {
  ...
  private endpoint() {
    return this.cfg.get('ETAX_ENDPOINT', 'https://etax.otr.tg/api/v1');
  }
  private isMock() {
    return this.cfg.get('ETAX_MOCK', 'true') === 'true';
  }
