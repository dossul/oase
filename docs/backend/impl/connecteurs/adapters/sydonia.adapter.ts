...
export interface SydoniaDeclaration {
  ...
  code_exoneration: string;  // code additionnel Sydonia (ex: '141')
...
/**
 * SydoniaAdapter — interface vers Sydonia World (OTR-CDDI).
 *
 * Opérations supportées :
 *  - verifierExoneration : vérifie si une exonération est enregistrée
 *  - getDeclarations     : récupère les déclarations d'un NIF
 *  - notifierDecision    : notifie Sydonia d'une approbation OASE
 *
 * En mode démo (SYDONIA_MOCK=true) → données simulées.
 */
@Injectable()
export class SydoniaAdapter {
    ...
    return this.cb.execute(
      ...
      async () => {
        ...
        const res = await fetch(
          `${this.endpoint()}/exonerations/verifier?nif=${nif}&code=${codeExo}`,
  ...
  async getDeclarations(nif: string, annee?: number): Promise<SydoniaDeclaration[]> {
    ...
    return this.cb.execute(
      ...
      async () => {
        ...
        const res = await fetch(
          `${this.endpoint()}/declarations?${params}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (!res.ok) throw new Error(`Sydonia HTTP ${res.status}`);
  ...
  private async getToken(): Promise<string> {
    // OAuth2 client_credentials — config chiffrée en DB
