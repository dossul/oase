...
### Session module extractif E1→E4 (2026-07-29 ~2h→14h)

- **INCIDENT prod (résolu)** : un `docker compose -f deploy/docker-compose.yml up oase-api` (compose DEV, même projet « deploy » que le compose PROD local) a **remplacé** les conteneurs prod `oase-api`/`oase-db` par des variantes dev (`oase-api-dev` ne pouvant démarrer : port 3000 hôte déjà pris par un bridge WhatsApp extérieur au projet). API down ~10 min. **Données intactes** (volume `oase_db_data` jamais touché). Récupération : suppression des conteneurs dev + `up -f docker-compose.local-prod.yml`. **Règle désormais absolue : ne JAMAIS utiliser `deploy/docker-compose.yml` sur le VPS — uniquement `docker-compose.local-prod.yml`.**
- **BUG #14 — Login UI accusait le mot de passe pendant les coupures serveur** — ✅ FIXED : tout échec (réseau, 5xx, validation) affichait « Identifiant ou mot de passe incorrect. Tentative X/5 » et incrémentait le compteur AVANT l'appel. Le compteur n'augmente plus que sur 401 (CREDENTIALS_INVALIDES) ; 400 → message de validation ; réseau/5xx → message technique distinct. `api.ts` propage désormais le code métier des 401 anonymes. Prouvé par `auth-login-ui.spec.ts` (mauvais mdp → 1/5, bons identifiants → /portail/dashboard), headless + headed.
- **BUG #15 — ParseIntPipe({optional:true}) renvoyait 400 sans query `annee`** — ✅ FIXED : `GET /flux-extractifs/*` sans `annee` → 400 au lieu de 200. Paramètre converti manuellement (`annee ? parseInt : undefined`).
- **Fragilités Playwright strict-mode récurrentes** (leçon documentée) : libellés en double KPI/titre de carte (`Permis enregistrés`), cellules dupliquées (`95 000 t`, `PE-2020-SNPT`, `193 000 t`), assertions de dialog non scopées (`getByRole('dialog')`), `v-select` cliqué sur l'`<input>` interne intercepté par son wrapper (cliquer le conteneur `.v-select`), `Escape` ne ferme pas toujours le `v-dialog` (cliquer le bouton fermer + attendre `not.toBeVisible`).
- **Récap extractif** : 8/8 specs extractif (E1→E4) headless + headed ; 52/52 suite recette headless ; 4/4 MFA isolées ; Jest 427/427 ; MFA global `enabled:false` vérifié post-run.

### Session complétude du périmètre (2026-07-30)

- **BUG #16 — Crash API (502) : cycle d'imports RbacMatriceService ↔ RbacMatriceController** — ✅ FIXED : pour que la matrice RBAC se liste elle-même, le service importait le contrôleur, qui importait le service → `design:paramtypes` cassé au bootstrap (« Nest can't resolve dependencies of the RbacMatriceController »), API down (502 nginx). Détecté immédiatement après déploiement par le smoke curl du endpoint. Fix : `@Inject(forwardRef(() => RbacMatriceService))` dans le contrôleur. Jest 323/323 sur le périmètre admin/RBAC, endpoint vérifié 200 en prod (104 entrées).
- **BUG #17 — Slash terminal fantôme dans la matrice RBAC** — ✅ FIXED : `@Post()`/`@Get()` **sans argument** enregistrent le path `/` chez NestJS → chemin calculé `/accords-siege/` au lieu de `/accords-siege`. Détecté par `rbac-matrice.service.spec.ts`. Fix : normalisation du slash terminal dans le service.
- **Fragilité test — Assertion figée sur des données de prod évolutives** : `rapprochements.spec.ts` exigeait `kpis.total === 30` (seed), mais chaque run TC-P4-01 approuve une demande réelle → total 31 au run suivant. Fix : assertion de cohérence (`≥ 30`, somme des statuts = total) au lieu d'un chiffre figé.
- **Fragilité test — `@click:row` Vuetify non déclenché en test** : TC-P7-03 cliquait une ligne de `v-data-table` pour ouvrir la fiche utilisateur ; le handler `click:row` ne se déclenchait pas de façon fiable (clic au centre de la ligne, interception par les cellules). Fix : clic sur le bouton « Permissions » de la ligne (`@click.stop`, chemin explicite) + attente du chargement réel de la table (skeleton).
- **Récap session** : 7 modules/vues rendus réels (reset email, accords de siège, 3 dashboards institutions, simulation décideur, rapprochements trésor, matrice RBAC admin + dictionnaire O2) — chacun prouvé en 3 modes (Jest/E2E API + Playwright headless + headed). Run final : **Jest 474/474 (33 suites) ; E2E headless 69/69 ; headed 15/15 sur les specs nouvelles/modifiées**. p6-opendata non rejouée (hors scope confirmé). Détail : `docs/qa/RAPPORT_COUVERTURE_TESTS_2026-07-28.md` v4.

### Incident démonstration publique (2026-07-31 ~2h) — 401 au login

- **Faits (audit prod, horodatés)** : un seul `LOGIN_ECHEC` à 01:59:46 sur `kossiwa.amele@texlome.tg` (`password_incorrect`), puis `LOGIN_SUCCES` du même compte à 02:01:43 avec `Oase@2026!`. Les 8 comptes de démo ont été retestés immédiatement : 8/8 → 200. **Aucune panne, aucun verrouillage, aucun mot de passe altéré** — l'échec venait de la saisie (espace de copier-coller ou faute de frappe), le backend a répondu correctement 401 `CREDENTIALS_INVALIDES`.
